...
public class GetBannerByIdQueryHandler(IBannerRepository bannerRepository,
    IHttpContextAccessor httpContextAccessor, IWebHostEnvironment env) : IRequestHandler<GetBannerByIdQuery, ServiceResponse<GetBannerByIdQueryOutputModel>>
{
    public async Task<ServiceResponse<GetBannerByIdQueryOutputModel>> Handle(GetBannerByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var r = FileHandler.GetFullDirectoryLocation(httpRequest, banner.Name,env);
        if (!r.Successful)
            return new("File not found!");

        GetBannerByIdQueryOutputModel result = new(banner.Id, FileHandler.GetFullDirectoryLocation(httpRequest, banner.Name,env).Response, banner.Title, banner.IsActive, banner.CreatedAt.ToString("g"));
