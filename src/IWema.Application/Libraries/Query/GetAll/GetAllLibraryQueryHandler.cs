...
public class GetAllLibraryQueryHandler(ILibraryRepository libraryRepository, IHttpContextAccessor httpContextAccessor, IWebHostEnvironment env) : IRequestHandler<GetAllLibraryQuery, ServiceResponse<GetAllLibraryQueryOutputModel>>
{
    public async Task<ServiceResponse<GetAllLibraryQueryOutputModel>> Handle(GetAllLibraryQuery request, CancellationToken cancellationToken)
    {
        ...
        List<GetLibraryModel> resultList = [.. libraries.Select(x => new GetLibraryModel(x.Id, FileHandler.GetFullDirectoryLocation(httpRequest, x.Name,env).Response, x.Description, x.Type, x.CreatedAt.ToString("g"))).OrderBy(o => o.Type)];
