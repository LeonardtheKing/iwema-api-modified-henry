...
namespace IWema.Application.ManagementTeam.Command.Update
{
    public class UpdateManagementTeamCommandValidator : AbstractValidator<UpdateManagementTeamCommand>
    {
        public UpdateManagementTeamCommandValidator()
        {
            ...
            When(command => !string.IsNullOrEmpty(command.ImageLink), () =>
            {
                RuleFor(command => command.ImageLink)
                    .Must(Link => Uri.TryCreate(Link, UriKind.Absolute, out var uriResult)
                        && (uriResult.Scheme == Uri.UriSchemeHttp || uriResult.Scheme == Uri.UriSchemeHttps))
                    .WithMessage("ImageLink must be a valid URL.");
            });

            // If ImageLink is empty, a file must be provided
            When(command => string.IsNullOrEmpty(command.ImageLink), () =>
            {
                RuleFor(command => command.File)
                    .NotNull()
                    .WithMessage("A file must be provided if ImageLink is empty.");
            });
